...
| 生成設定 | 使用目的 |
| --- | --- |
| [gen-config.yaml](gen-config.yaml) | Redis の基本設定と `awsenv` → `env`、`service` → `team` のタグ名変換 |
| [gen-config-multi-region.yaml](gen-config-multi-region.yaml) | 東京・大阪の Redis を集約し、本番または検証環境から監視除外タグのある対象を除く |
