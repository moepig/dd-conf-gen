...
| `data.secrets` | map | | テンプレートに渡す Secrets Manager の参照定義 |
...
各ファイルは出力先と同じディレクトリに作成した一時ファイルから置換する。シークレット参照のない出力では既存ファイルのアクセス権を引き継ぐ。既存のシンボリックリンクはリンク先を更新する。リンク先が存在しないシンボリックリンクはエラーとする。

新規ファイルのアクセス権は `0644` にプロセスの `umask` を適用する。既存ファイルのパーミッションビットは保持する。`data.secrets` を指定した出力は、新規・既存ファイルともに `0600` とする。所有者・ACL・拡張属性の保持は保証しない。
...
- `.Secrets`: `data.secrets` で定義した名前をキーとする取得値の文字列マップ
...
`quote` 関数は文字列を二重引用符付きの YAML スカラーへ変換する。シークレット参照の設定と保存時のアクセス権は、[Secrets Manager のシークレット参照](docs/secrets.md) を参照。
